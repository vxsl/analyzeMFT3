...
def parse_record(raw_record, options):
    ...
    if (
        record["seq_number"] == raw_record[510:512]
        and record["seq_number"] == raw_record[1022:1024]
    ):
        raw_record = "%s%s%s%s" % (
            raw_record[:510],
            record["seq_attr1"],
    ...
    while read_ptr < 1024:
        ...
        if options.debug:
            print(
                "Attribute type: %x Length: %d Res: %x"
                % (atr_record["type"], atr_record["len"], atr_record["res"])
            )

        if atr_record["type"] == 0x10:  # Standard Information
            if options.debug:
                print(
                    "Stardard Information:\n++Type: %s Length: %d Resident: %s Name Len:%d Name Offset: %d"
                    % (
            ...
            si_record = decode_si_attribute(
                raw_record[read_ptr + atr_record["soff"] :], options.localtz
            )
            record["si"] = si_record
            if options.debug:
                print(
            ...
            if atr_record["res"] == 0:
                al_record = decode_attribute_list(
                    raw_record[read_ptr + atr_record["soff"] :], record
                )
                record["al"] = al_record
                if options.debug:
            ...
            fn_record = decode_fn_attribute(
                raw_record[read_ptr + atr_record["soff"] :], options.localtz, record
            )
            ...
            if fn_record["crtime"] != 0:
                if options.debug:
                    print(
                        "\tCRTime: %s MTime: %s ATime: %s EntryTime: %s"
                        % (
            ...
            object_id_record = decode_object_id(
                raw_record[read_ptr + atr_record["soff"] :]
            )
            ...
            volume_info_record = decode_volume_info(
                raw_record[read_ptr + atr_record["soff"] :], options
            )
            ...
            if atr_record["name"] != "":
                ...
                record["ads"] += 1
            if atr_record["res"] == 0:
                data_attribute = decode_data_attribute(
                    raw_record[read_ptr + atr_record["soff"] :], atr_record
                )
            else:
                data_attribute = {
...
def mft_to_csv(record, ret_header, options):
    ...
    csv_string = [
        record["recordnum"],
        decode_mft_magic(record),
        decode_mft_isactive(record),
    ...
    if record["fncnt"] > 0:
        csv_string.extend(
            [str(record["fn", 0]["par_ref"]), str(record["fn", 0]["par_seq"])]
        )
    else:
    ...
    for record_str in ["si", "al"]:
        csv_string.append("True") if record_str in record else csv_string.append(
            "False"
        )
...
def decode_mft_header(record, raw_record):
    ...
    record["recordnum"] = struct.unpack("<I", raw_record[44:48])[
        0
...
def decode_atr_header(s):
    ...
    if d["res"] == 0:
        ...
        d["run_off"] = struct.unpack("<H", s[32:34])[
            0
        ]  # wDataRunOffset (in clusters, from start of partition?)
...
def unpack_dataruns(datarun_str):
    ...
    class Lengths(ctypes.Union):
        _fields_ = [("b", LengthBits), ("asbyte", c_uint8)]
    ...
    while True:
        ...
        bit_len = bitparse.parse_little_endian_signed(
            datarun_str[pos : pos + lengths.b.lenlen]
        )
        ...
        if lengths.b.offlen > 0:
            offset = bitparse.parse_little_endian_signed(
                datarun_str[pos : pos + lengths.b.offlen]
            )
...
def decode_si_attribute(s, localtz):
    d = {
        "crtime": mftutils.WindowsTime(
            struct.unpack("<L", s[:4])[0], struct.unpack("<L", s[4:8])[0], localtz
        ),
        "mtime": mftutils.WindowsTime(
            struct.unpack("<L", s[8:12])[0], struct.unpack("<L", s[12:16])[0], localtz
        ),
        "ctime": mftutils.WindowsTime(
            struct.unpack("<L", s[16:20])[0], struct.unpack("<L", s[20:24])[0], localtz
        ),
        "atime": mftutils.WindowsTime(
            struct.unpack("<L", s[24:28])[0], struct.unpack("<L", s[28:32])[0], localtz
        ),
...
def decode_fn_attribute(s, localtz, _):
    ...
    d = {
        ...
        "atime": mftutils.WindowsTime(
            struct.unpack("<L", s[32:36])[0], struct.unpack("<L", s[36:40])[0], localtz
        ),
        ...
        objstr = "-".join(
            map(
                ...
                list(
                    map(
                        binascii.hexlify,
                        (s[0:4][::-1], s[4:6][::-1], s[6:8][::-1], s[8:10], s[10:]),
                    )
                ),
            )
        )
...
def anomaly_detect(record):
    if record["fncnt"] > 0:
        ...
        try:
            if (
                record["si"]["atime"].dt > record["si"]["mtime"].dt
                and record["si"]["atime"].dt > record["si"]["crtime"].dt
            ):
